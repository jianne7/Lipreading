...
def prepare_device(n_gpu: int, keep_reproducibility=False):
    ...
    if n_gpu == 0:
        ...
        if keep_reproducibility:
            ...
            torch.torch.use_deterministic_algorithms(True)  # configures PyTorch only to use deterministic implementation
        else:
...
class Inferencer:
    ...
    def __call__(self):
        ...
        for noisy, name in tqdm(self.dataloader, desc="Inference"):
            ...
            name = name[0]
            enhanced = self.full_band_crm_mask(noisy.to(self.device), inference_args)
